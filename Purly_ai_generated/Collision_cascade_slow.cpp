...
#define DEACTIVATION_THRESHOLD 1.0f // Lower threshold for smoother stopping
#define GROUND_FRICTION 0.88f       // Increased ground friction
#define WALL_FRICTION 0.85f
#define AIR_RESISTANCE 0.95f // Increased air resistance
...
void ResolveBallCollision(Ball *ball1, Ball *ball2)
{
    ...
    if (distance < minDistance)
    {
        ...
        float restitution = 0.6f; // Reduced restitution for less bounciness
                       ...
                       float groundLevel, int screenWidth, int screenHeight)
{
    ...
    if (ball->position.y + ball->radius >= groundLevel)
    {
        ...
        ball->velocity.y *= -0.5f; // Reduced bounce
    ...
    ball->velocity.y *= friction; // Added vertical air resistance
...
void InitializeBalls(Ball balls[], int screenWidth, int screenHeight)
{
    for (int i = 0; i < MAX_BALLS; i++)
    {
        balls[i] = (Ball){
            ...
            .velocity = {(float)GetRandomValue(-100, 100), // Reduced initial velocity
                         (float)GetRandomValue(50, 200)},  // Reduced initial velocity
...
int main(void)
{
    ...
    SetTargetFPS(60); // Reduced frame rate for smoother animation
    ...
    while (!WindowShouldClose())
    {
        ...
        for (int i = 0; i < MAX_BALLS; i++)
        {
            if (balls[i].active)
            {
                UpdateBallPhysics(&balls[i], 0.4f, AIR_RESISTANCE, // Reduced gravity
